...
## 2026-05-31 - Fix Unhandled IsADirectoryError Exception
**Vulnerability:** The `/results` and `/update_action` endpoints caught `FileNotFoundError` but failed to catch `IsADirectoryError` (or `PermissionError`) when attempting to `open()` the dynamically constructed JSON path. If an attacker created a directory matching the expected `_analysis.json` file pattern, the unhandled `IsADirectoryError` would bubble up as a 500 Internal Server Error, potentially leaking system internals.
**Learning:** When interacting with the filesystem using user-influenced paths, trapping only `FileNotFoundError` is insufficient. The operating system can raise other `OSError` subclasses like `IsADirectoryError` or `PermissionError`.
**Prevention:** Always catch the broader `OSError` base class instead of just `FileNotFoundError` when attempting to open files, ensuring that any filesystem-level access failure degrades gracefully into a 404 Not Found or generic 400 Bad Request error.
