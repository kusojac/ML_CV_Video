...
## 2026-05-30 - Add Content-Security-Policy Header
...
## 2026-06-26 - Prevent Path Traversal and Arbitrary File Reads with Absolute Path Checking
**Vulnerability:** The application's `secure_path` function validated input to prevent `..` (directory traversal) and `\x00` (null byte) attacks. However, it failed to block absolute paths (e.g., `/etc/passwd` or `C:/Windows/system32`). Because Python's `os.path.join()` function will discard the base path and return the absolute path if it is provided as the second argument, an attacker could supply an absolute path as the `video_path` to read or overwrite arbitrary files on the filesystem, leading to a critical security vulnerability.
**Learning:** Path validation in Python must check for both relative directory traversals (`..`) and absolute paths (`os.path.isabs()`). Omitting the absolute path check allows attackers to bypass intended directory restrictions entirely.
**Prevention:** Always use `os.path.isabs(path)` in path validation logic to ensure that user-supplied paths are strictly relative to the intended base directory.
