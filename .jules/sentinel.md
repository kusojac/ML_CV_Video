...
## 2026-06-01 - Prevent DoS via Resource Exhaustion in Unbounded Background Tasks
**Vulnerability:** The `/analyze` endpoint lacked a limit on the number of concurrent video processing background tasks it could spawn. A malicious actor could exploit this by sending many analyze requests, spawning unlimited ML pipeline instances, exhausting server memory and CPU, leading to a Denial of Service (DoS).
**Learning:** FastAPI's `BackgroundTasks` executes tasks immediately upon yielding the response without built-in concurrency limits or queuing mechanisms by default. Relying on this for heavy ML tasks poses a severe resource exhaustion risk.
**Prevention:** Always enforce strict concurrency limits or use a dedicated task queue (like Celery or Redis Queue) for computationally expensive operations to gracefully limit payload size at the framework layer. Use a simple active job counter and return a 429 Too Many Requests status code when limits are reached to gracefully handle excess load.
