...
def postprocess_yolo_ball(output, original_img_shape, input_size=(640, 640),
...
def postprocess_yolo_coco(output, original_img_shape, input_size=(640, 640),
...
def postprocess_yolo_output(output, original_img_shape, input_size=(640, 640),
                            conf_threshold=0.25, nms_threshold=0.45, target_class_id=None):
    """
    Backward compatible dispatcher for yolo output processing.
    """
    sq_output = np.squeeze(output)
    if sq_output.shape[0] < sq_output.shape[1]:
        sq_output = sq_output.T

    num_features = sq_output.shape[1]

    if num_features == 5:
        return postprocess_yolo_ball(output, original_img_shape, input_size, conf_threshold, nms_threshold)
    elif num_features == 84:
        return postprocess_yolo_coco(output, original_img_shape, input_size, conf_threshold, nms_threshold, target_class_id)
    else:
        return np.array([]).reshape(0,4), np.array([]), np.array([])