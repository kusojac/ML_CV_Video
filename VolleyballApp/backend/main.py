...
async def analyze_video(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    # Enforce concurrency limit to prevent DoS via CPU/Memory exhaustion
    MAX_CONCURRENT_JOBS = 5
    active_jobs = sum(1 for j in analysis_jobs.values() if j.get("status") in ("pending", "processing"))
    if active_jobs >= MAX_CONCURRENT_JOBS:
        raise HTTPException(status_code=429, detail="Too many concurrent analysis jobs. Please try again later.")
