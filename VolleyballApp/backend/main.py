...
# Globally initialized ProcessPoolExecutor to avoid per-request instantiation overhead
_json_pool = concurrent.futures.ProcessPoolExecutor()
...
def get_results(video_path: str = Query(..., max_length=2048)):
    ...
    json_str = _json_pool.submit(json.dumps, data).result()
